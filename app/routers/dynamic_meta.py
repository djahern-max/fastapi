# app/routers/dynamic_meta.py - Simplified version using your existing config
...
from ..config import settings  # Use your existing config

router = APIRouter(tags=["Dynamic Meta Tags"])
logger = logging.getLogger(__name__)


# Use your existing frontend URL configuration
def get_base_url():
...
async def video_meta(video_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        meta_data = {
            ...
            or f"Amazing {video.video_type.replace('_', ' ').title()} by {creator_name}",
